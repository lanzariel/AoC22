...
def get_least_d(real_raw_g, starting, ending, shift):
    g = Grid(real_raw_g)
    for it in range(shift):
        g.move()
    processing = [starting]
...
to_end = get_least_d(grid_raw, (-1,0), (rows-1,cols-1), 0) + 2
to_back = get_least_d(grid_raw, (rows, cols-1), (0,0), to_end) + 2
to_finish = get_least_d(grid_raw, (-1, 0), (rows-1, cols-1), to_end+to_back)+2
print(to_end + to_back+ to_finish)
